...
- **`scripts/`** — futtatható Python (19 `.py` + 9 `.sh`), nincs build, nincs test framework
...
- **Conda env:** `eval-hu` Python 3.11-gyel (`/home/openclaw/anaconda3/envs/eval-hu`). **Python 3.12 nem jó** (`deepeval` kompatibilitás). Ha `which pip` `/usr/bin/pip`, a `conda activate eval-hu` nem futott le.
...
python scripts/download_ud_hungarian.py        # UD Hungarian (CoNLL-U)

# Benchmark futtatás — ugyanaz a parancs RESUME-ol is
python scripts/run_hulu.py --model qwen3.5:4b            # első ind. vagy folytatás
...
python scripts/run_hulu.py --model gpt-oss:20b-cloud --backend openai --base-url http://localhost:11434/v1 --limit 10  # smoke test
...
- Bármilyen Ollama hiba (429, 5xx, timeout, connection error, modell nem található) → `OllamaFatalError` → futás AZONNAL megáll.
- State atomi write (`tmp` + `os.replace`) `state/{model_safe}-{mode}/hulu.json`-ba (`scripts/checkpoint.py`).
- `results/{model}/hulu_results.jsonl` append-only, minden item után `flush` + `fsync` (`scripts/run_hulu.py:220-222`).
...
- **Modellnév → mappa:** `model.replace(":", "-").replace("/", "-") + f"-{mode}"` (`scripts/run_hulu.py:86`). Pl. `qwen3.5:4b` + `nothink` → `qwen3.5-4b-nothink`. Mindig ezt alkalmazd fájlútvonalnál.
- **Cloud modell-pool:** a forrása a `scripts/queue_all_benchmarks.sh` `MODELS` tömb (nem a wiki) — ott van az aktuális lista és a `:cloud` suffix konvenció. Aktív default például `minimax-m3:cloud`. Ha egy korábban RETIRED-nek írt modell (pl. `qwen3-next:80b-cloud`) még szerepel a scriptben, a script az igazság — ne távolítsd el anélkül, hogy ellenőriznéd.
- **Cloud modell rákérdezés:** ha a modell nincs helyben és nem `:cloud` suffix-szel végződik, a script megkérdezi, cloud-ként akarod-e (`scripts/run_hulu.py:227-235` környéke).
- **Ollama kliens beállítások** (`scripts/stop_on_error.py`): `temperature: 0.0`, `stream: False`, `num_predict` alapértelmezetten `4096` (nothink), think módban `16384`. Ne változtasd meg — az eredmények reprodukálhatósága függ tőle.
- **Composite score súlyok:** 40% statisztikai + 40% generatív + 20% nyelvészeti, hardcoded `scripts/aggregate_results.py:28` (`W_STAT, W_GEN, W_LING`). STAT=`[hulu, mmlu_hu]`, GEN=`[hugme, mt_bench_hu]`, LING=`[ud_hungarian]`. Ha egy dimenzió hiányzik, a súlyok újraosztódnak — NE változtasd meg.
...
- **Benchmark-leírás konvenció (KÖTELEZŐ):** minden riportban, **minden benchmark-szekció elejére** be KELL tenni egy rövid (2-4 soros) leírást arról, hogy az adott benchmark **mit tesztel** (milyen képességet mér, milyen formátumban, milyen kimenettel). Ez a kötelező fejléc a HuLU, MMLU-HU, HuGME, MT-Bench-HU és UD Hungarian szekciókban is — nem opcionális. (Minta: `wiki/reports/report-2026-07-14.md` "Per-benchmark eredmények" szakasz, ahol minden szekció tartalmazza a "**Mit tesztel:**" blokkot.)
- **Kvantálás konvenció (KÖTELEZŐ):** minden riportban a modellek **kvantálási szintjét** kötelezően szerepeltetni kell (külön "Modell-kvantálás" szakasz a fejléc után, vagy oszlop a modell-táblázatban). **Új modell futtatása előtt mindig rá kell kérdezni a felhasználóra: milyen kvantálással fut (pl. q4_K_M, fp16, awq, stb.), vagy cloud-e.** Ha a modell Ollama Cloud alatt fut (így a lokális kvantálás nem ismert), akkor `ollama-cloud` értéket kell beírni — nem lehet üresen hagyni. (Minta: `wiki/reports/report-2026-07-14.md` "Modell-kvantálás" szakasz — minden modell `ollama-cloud`.)
- **Több kvantálás ugyanarra a modellre:** előfordulhat, hogy egy modellt több kvantálással is futtatunk benchmarkra. Ilyenkor minden (modell × kvantálás) kombináció **külön sor** a riportban (külön modell-bejegyzés, pl. `qwen3.5:4b-q4_K_M` és `qwen3.5:4b-fp16`), és a mappa-útvonalban is különválik (a modell-bélyegző tartalmazza a kvantálást: `scripts/run_hulu.py:86` szerinti `model_safe` + `-{mode}` helyett `model-<kvantálás>-{mode}` formában — ellenőrizd a scriptet, ha szükséges). A "Modell-kvantálás" szakaszban is külön sor minden kombinációnak.
- **Backend konvenció (KÖTELEZŐ a riportban):** a benchmarkok futtathatók közvetlen Ollama backenden (`--backend ollama`, default) vagy OpenAI-kompatibilis backenden (`--backend openai --base-url ...`, pl. helyi Ollama `/v1`, később llama-server). Ha a futtatás **nem** közvetlen Ollama volt, a riportban a modellnél fel kell tüntetni a backendet (pl. "OpenAI /v1 @ localhost:11434"). A `results/*.jsonl` sorai tartalmazzák a `"backend"` mezőt — ebből visszakereshető. A checkpoint/stop-on-error rendszer backend-független (közös `FatalBackendError`).
