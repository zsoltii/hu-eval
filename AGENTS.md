...
- **Teljes összesítő riport (KÖTELEZŐ):** a `wiki/reports/report.md` fájl a projekt **kanonikus teljes összesítő riportja**, amely mindig az összes eddigi modell (cloud + lokális) eredményeit tartalmazza. Új modell hozzáadásakor a `wiki/reports/report.md`-t is frissíteni KELL (nem elég csak a `wiki/reports/` dátumozott riportot frissíteni). A `wiki/reports/` dátumozott riportok archív jellegűek; a `wiki/reports/report.md` a mindig aktuális, teljes riport. Ha új modell eredményei bekerülnek, a `wiki/reports/report-YYYY-MM-DD.md` frissítése után a `wiki/reports/report.md`-t is szinkronizálni kell (vagy szimbolikus linkként hivatkozni a legfrissebb dátumozott riportra).
...
## KÖTELEZŐ: Soha ne állítsd le a lokális AI szervert!

A lokális AI szerver (Ollama `localhost:11434`, llama-server `localhost:8080`, stb.) **futó benchmark futás alatt is fut**. Ezek a szerverek betöltött modellekkel több GB RAM/VRAM foglalnak, és a betöltés percekig tart. A szerver leállítása = a futó benchmark azonnali megszakítása, az összes eddigi állapot érvénytelenné válhat, és az újraindítás + modell újratöltése 5-10 perc.

**Szabály: SOHA ne `kill`, `pkill`, `kill -9` vagy bármilyen módon leállítsd a futó AI szervert!** Ha konfigurációs változtatás szükséges (pl. context méret, port, stb.), kérdezd meg a felhasználót először — a felhasználó maga fogja újraindítani a szervert a megfelelő paraméterekkel.

Ez alól kivétel NINCS. Még ha a benchmark látszólag "beragadt" is, először diagnosztizáld (log-ok, state fájl, processz állapot), és jelentse a felhasználónak. Ne foglald magadnak a szerver újraindítását.
